      ...
      must_support_elements.select do |element_definition|
        resources.none? do |resource|
          ...
          value_found = find_a_value_at(resource, path) do |value|
            if value.instance_of?(SHCVaccinationTestKit::PrimitiveType) && ms_extension_urls.present?
              urls = value.extension&.map(&:url)
              has_ms_extension = (urls & ms_extension_urls).present?
            end

            unless has_ms_extension
              value = value.value if value.instance_of?(SHCVaccinationTestKit::PrimitiveType)
